                ...
                [itemProvider loadItemForTypeIdentifier:@"public.url"
                                      ...
                                      completionHandler:^(id<NSSecureCoding>  _Nullable item, NSError * _Null_unspecified error) {
                                          if ([(NSObject *)item isKindOfClass:[NSURL class]]) {
                                              ...
                                              [userDefaults setValue:((NSURL *)item).absoluteString forKey:@"publicURL"];
                                              //用于标记是新的分享
...
- (NSArray *)configurationItems {
    ...
    SLComposeSheetConfigurationItem *itemtest = [[SLComposeSheetConfigurationItem alloc] init];
    itemtest.title = @"title test";
    itemtest.value = @"value test";
    itemtest.tapHandler = ^{
        
    };
    itemtest.valuePending = NO;//加载指示器
    [items addObject:itemtest];
